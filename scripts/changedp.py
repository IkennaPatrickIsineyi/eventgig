...
def getName():
    ...
    fileEx = formData.getvalue('extension')
    dp = formData["dp"]
    return username, dp, sessionid, fileEx


def savePic(db, cursor, username, dp, fileEx):
    try:
        dpFn = os.path.basename(username+'.'+fileEx)
        ...
        state = {'status': 'failed', 'dpFn': dpFn, 'username': username}
...
def email_result(username, email, subject):
    ...
    with smtplib.SMTP_SSL('smtp.gmail.com', 465, context=context) as server:
        ...
        textMsg = """{}, your profile picture has been changed \n\n""".format(
            username)
